...
public class AlgorithmController {
    ...
    public Result<List<TaAllocation>> assignGreedy() {
        ...
        for (int i = 0; i < 4; i++) {
                ...
                for (TeachingSession session : moduleSessions) {
                    ...
                    for (Ta ta : sortedCandidates) {
                        ...
                        for (TimetableEntry entry : sessionEntries) {
                            ...
                            if (!taAllocatedEntries.containsKey(ta.getId())) {
                                taAllocatedEntries.put(ta.getId(), new ArrayList<>());
                            }
                            taAllocatedEntries.get(ta.getId()).add(entry);
                        }
            ...
            //check whether all TimeTableEntry have been allocated
